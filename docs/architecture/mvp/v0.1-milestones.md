...
2. **Kithara skeleton** — feature-first API layout, module registry, persistence, **OTel bootstrap** (`bardie.kithara`)
3. **Bes + JWT sessions** — discovery, authenticate/refresh (Bes mints JWT); bootstrap admin; Kithara JWKS verify; OTel `bardie.auth.bes`
4. **Source module protocol** — gRPC + FIFO PCM proof with Magpie (`StartTrack` / `StopTrack`); OTel `bardie.source.magpie`
